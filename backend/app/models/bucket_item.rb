class BucketItem < ApplicationRecord
  ...
  validates :target_year, presence: true, numericality: { only_integer: true }
  ...
  def target_year_within_bucket_range
    ...
    unless target_year.between?(min_year, max_year)
      errors.add(:target_year, "must be between #{min_year} and #{max_year} (age #{time_bucket.start_age}-#{time_bucket.end_age}) for this bucket")
    end
  end
